...
<head>
	...
	<script type="text/javascript"> 
		...
		function onDelete(id){
			if(confirm("确定要删除吗？")){
				location.href = "${basePath}/banner/delete?id="+id;
			}
		}
	</script>
		...
		<div class="tools">
			<div class="toolbar">
				<input type="button" value="＋新增" class="tbtn click"
					onclick="location.href='${basePath}/banner/toEdit'" />
			</div>
		</div>
			...
			<tbody>
				<c:forEach items="${page.list}" var="banner">
					<tr>
						...
						<td>${banner.url}</td>
						<td><img src="${banner.background}" height="20px" width="20px"/></td>
						<td>${banner.sortIndex}</td>
						<td>${banner.createDate}</td>
						<td>
							<a href="banner/toEdit?id=${banner.id}" class="tablelink">修改</a>
							<a href="javascript:void(0);" onclick="onDelete('${banner.id}')" class="tablelink">删除</a> 
						</td>
					</tr>
				</c:forEach>
			</tbody>
