...
<head>
	...
	<script type="text/javascript"> 
		...
		function gotoPage(pageNumber) {
			...
			var title = $('#title').val();
			location.href = "${basePath}/content/relatedArticles/list?pageNumber=" + pageNumber +"&title="+title;
		}
		...
		function confirm(){
			...
			if(chk_value.length==0 ){
				...
				if(chk_value.length>3){
					...
					$('#relatedArticless').val(chk_value);
				} */
			}
			parent.closeRelatedArticless(chk_value.toString(), chk_name.toString());
		}
...
<body>
	...
	<div class="rightinfo">
		<form action="content/relatedArticles/list" id="myForm">
		...
		<table class="tablelist">
			...
			<tbody>
				<c:forEach items="${page.list}" var="content">
					<tr>
						<th>
							<input type="checkbox" name="checkbox" value="${content.id}"/>
							<input type="hidden" id="${content.id}" value="${content.title}"/>
						</th>
						<td>${content.title}</td>
